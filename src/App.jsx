...
// Fake video thumbnail with a play affordance.
function VideoThumb({ tint, timestamp, big = false }) {
  return (
    <div
      className={`relative flex-shrink-0 overflow-hidden rounded-xl bg-gradient-to-br ${tint} ring-1 ring-white/10 ${
        big ? "aspect-video w-full" : "h-16 w-24"
      }`}
    >
      ...
      <div className="absolute inset-0 opacity-20">
        ...
        <div className="absolute left-1/2 top-1/2 h-8 w-8 -translate-x-1/2 -translate-y-1/2 rounded-full border border-white" />
      </div>
      <div className="absolute inset-0 grid place-items-center">
        <div className={`grid place-items-center rounded-full bg-black/40 backdrop-blur ${big ? "h-14 w-14" : "h-8 w-8"}`}>
          <svg viewBox="0 0 24 24" className={big ? "h-6 w-6" : "h-3.5 w-3.5"} fill="white">
      ...
      <div className="mt-8">
        ...
        <ul className="space-y-2">
          {clips.map((clip) => (
            ...
            >
              <VideoThumb tint={clip.tint} timestamp={clip.timestamp} />
...
function ReviewQueue({ clips, index, decisions, onDecide, onBack }) {
  const clip = clips[index]
  const [mode, setMode] = useState("idle") // idle | reassigning
  const [pick, setPick] = useState("")

  // Reset local UI when the clip changes.
  ...
  const progress = Math.round((index / clips.length) * 100)
  ...
  useEffect(() => {
    function onKey(e) {
      // While the roster <select> is focused, still allow save/cancel but
      // leave its own typeahead and arrow-key navigation alone.
      if (e.target.tagName === "SELECT") {
        if (e.key === "Enter") { e.preventDefault(); reassign() }
        if (e.key === "Escape") { setMode("idle"); setPick("") }
        return
      }
      if (mode === "idle") {
        ...
        if (e.key === "Enter") { e.preventDefault(); reassign() }
        if (e.key === "Escape") { e.preventDefault(); setMode("idle"); setPick("") }
      }
    }
  ...
  return (
    <div className="mx-auto max-w-2xl px-5 py-8 sm:px-8 sm:py-12">
      ...
      <div className="flex items-center justify-between">
        ...
        <p className="font-mono text-xs text-slate-500">{decisions.length} verified</p>
      </div>
      ...
      <div className="mt-6 overflow-hidden rounded-2xl border border-white/10 bg-[#141a22]">
        <div className="p-4 sm:p-5">
          <VideoThumb tint={clip.tint} timestamp={clip.timestamp} big />
        </div>
        ...
        <div className="space-y-5 px-5 pb-6 sm:px-6">
          ...
          {mode === "idle" ? (
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <button
                onClick={confirm}
                className="inline-flex items-center justify-center gap-2 rounded-xl bg-green-500 px-5 py-3.5 text-sm font-semibold text-slate-950 transition hover:bg-green-400 active:scale-[0.98]"
              >
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/15 bg-white/5 px-5 py-3.5 text-sm font-semibold text-white transition hover:bg-white/10 active:scale-[0.98]"
              >
      ...
      <p className="mt-4 text-center font-mono text-[11px] text-slate-600">
        Press C to confirm, R to reassign. Enter saves, Esc cancels.
      </p>
    </div>
  )
}
...
function Complete({ decisions, onSend, sent, onRestart }) {
    ...
    <div className="mx-auto max-w-2xl px-5 py-8 sm:px-8 sm:py-12">
      <div className="text-center">
        ...
        <p className="mt-2 text-sm text-slate-400">
          You reviewed {decisions.length} flagged {decisions.length === 1 ? "clip" : "clips"}. Here is the summary.
        </p>
      </div>
...
export default function App() {
  ...
  function handleDecide(decision) {
    ...
    setDecisions(next)
    if (index + 1 >= FLAGGED_CLIPS.length) {
      setStep("done")
    } else {
      setIndex(index + 1)
    }
  ...
  // Step back to the previous clip and drop its decision so it can be redone.
  function goBack() {
    if (index === 0) return
    setIndex(index - 1)
